...
async function getLunarMetadata(): Promise<any> {
    return axios.post("https://api.lunarclientprod.com/launcher/launch", {
        hwid: "not supplied",
        os: "win32",
        arch: "x64",
        version: "1.8.9",
...
async function fetchCosmeticsIndex(): Promise<Cosmetic[]> {
    ...
    const entry = assetsIndex.files.find(f => f.path == "assets/lunar/cosmetics/index");
    const url = assetsIndex.baseUrl + entry.sha1;

    return axios.get(url).then(res => res.data).then(data => data.split("\n").map(line => {
        const parts = line.split(" ");
        return {
            id: parseInt(parts[0]),
            resource: parts[8],
...
export interface Cosmetic {
    ...
    type: string,
    animated: boolean,
    category: 'cloak' | 'hat' | 'bodywear' | 'neckwear' | 'mask' | 'bandanna' | 'dragon_wings' | 'backpack',
    indexType: string
}
