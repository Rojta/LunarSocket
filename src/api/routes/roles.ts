...
rolesRouter.patch('/', auth, async (request: Request<{}, {}, RolesRequestBody>, response) => {
	if (typeof request.body.player !== 'string' || typeof request.body.role !== 'string') return response.sendStatus(400);

	if (request.body.offline === true) {
		const addDashes = i => `${i.substr(0, 8)}-${i.substr(8, 4)}-${i.substr(12, 4)}-${i.substr(16, 4)}-${i.substr(20)}`;
		const uuid = addDashes((await axios.get(`https://api.mojang.com/users/profiles/minecraft/${request.body.player}`)).data.id);
		const player = await DatabaseManager.instance.database.getPlayer(uuid);
		if (!player) return response.sendStatus(404);
		player.role = (await getRole(request.body.role)).default ? 'default' : request.body.role;
...
rolesRouter.get('/', auth, async (req, res) => res.send((await getConfig()).roles));
