...
statsRouter.get('/', auth, async (request, response) => {
	const averageConnected = Math.round(Object.values(st.onlinePlayers).reduce((p, c) => p + c, 0) / Object.values(st.onlinePlayers).length);
	...
	const stats = {
		...
		rankRepartition: await DatabaseManager.instance.database.getRoleDistribution(),
		status: {
			ramUsage: {
