...
export default class Redis extends Database {
	...
	private queue: CallQueue<{ uuid: string; player: DatabasePlayer }, (player: { uuid: string; player: DatabasePlayer }) => Promise<void>>;
	...
	private async init(): Promise<void> {
		...
		if ((await this.client.exists('players')) === 0) await this.client.json.set('players', '$', {});
	...
	private async emptyQueue({ uuid, player }: { uuid: string; player: DatabasePlayer }): Promise<void> {
	...
	public async setPlayerRaw(uuid: string, player: DatabasePlayer): Promise<void> {
		...
		return await this.client.json.set('players', uuid, player as { [key: string]: any });
	}
	...
	public async getCustomCosmetics(): Promise<CustomCosmetic[]> {
		return (await this.client.json.get('customCosmetics')) as unknown as CustomCosmetic[];
	}

	// TODO: Role Distribution for Redis
	// skipcq
