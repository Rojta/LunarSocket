...
export default class Mongo extends Database {
	...
	public async getCustomCosmetics(): Promise<CustomCosmetic[]> {
		return (await this.customCosmeticsCollection.find().toArray()) as unknown as CustomCosmetic[];
	}

	public async getRoleDistribution(): Promise<{ [role: string]: number }> {
		const roleNames = Object.keys((await getConfig()).roles).filter(role => role !== 'default');
		const roles = {};
		await Promise.all(
			roleNames.map(role =>
