...
import { IncomingPacketHandler, OutgoingPacketHandler } from "../packets/PacketHandlers";
...
export default class Player {
	...
	public constructor(socket: WebSocket, handshake: Handshake, cosmetics: Cosmetic[]) {
		...
		for (let id = 0; id < 400; id++) this.emotes.fake.push(id);

		for (const cosmetic of cosmetics) this.cosmetics.fake.push({ id: cosmetic.id, equipped: false });
		...
		// isn't ready yet
		const incomingMessageQueue = new CallQueue<Buffer, (data: Buffer) => Promise<void>>(handleIncomingMessage);
		...
		(async () => {
			...
			const outgoingEvents = await readdir(join(process.cwd(), "dist", "player", "events", "outgoing"));
			for (const event of outgoingEvents) {
				...
				this.outgoingPacketHandler.on(
					...
					event.replace(".js", ""),
					async packet => {
						const handler = await import(join(process.cwd(), "dist", "player", "events", "outgoing", event));
			...
			const incomingEvents = await readdir(join(process.cwd(), "dist", "player", "events", "incoming"));
			for (const event of incomingEvents) {
				...
				this.incomingPacketHandler.on(
					...
					event.replace(".js", ""),
					async packet => {
						const handler = await import(join(process.cwd(), "dist", "player", "events", "incoming", event));
			...
			this.fakeSocket = new WebSocket("wss://assetserver.lunarclientprod.com/connect", {
				headers: { ...handshake }
			});
	...
	public getPlayerInfo() {
		...
		if (permissions.includes("*") || permissions.includes("customization.displayColor")) color = this.customization.displayColor;
		if (permissions.includes("*") || permissions.includes("customization.displayPlusColor")) plusColor = this.customization.displayPlusColor;

		return {
			cosmetics: [...this.cosmetics.fake, ...this.cosmetics.owned].filter(c => c.equipped),
